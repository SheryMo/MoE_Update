...
class LinearGLUExperts(nn.Module):
    ...
    def __init__(
        ...
        self.expert_routing_map = list(range(self.num_experts))  # [0, 1, 2, 3, ..., N-1]
    ...
    def forward_all_experts(self, input):
        outputs = []
        for i in range(self.num_experts):
            actual_idx = self.expert_routing_map[i]
            gate = self.act_fn(
                F.linear(
                    input,
                    self.weight_gate[actual_idx],
                    self.bias_gate[actual_idx] if self.bias_gate is not None else None,
                )
            )
            up = F.linear(
                input,
                self.weight_up[actual_idx],
                self.bias_up[actual_idx] if self.bias_up is not None else None,
            )
            down = F.linear(
                gate * up,
                self.weight_down[actual_idx],
                self.bias_down[actual_idx] if self.bias_down is not None else None,
            )
            outputs.append(down)
        return outputs

    def remap_expert_idx(self, from_idx, to_idx):
        """
        将 from_idx 的调用映射到 to_idx
        """
        self.expert_routing_map[from_idx] = to_idx
    def remap_expert(self, new_routing_map):

        self.expert_routing_map = new_routing_map

    #     self.remove_unused_experts_with_placeholder_1x1()

    # def remove_unused_experts_with_placeholder_1x1(self):
    #     """
    #     将未被 expert_routing_map 使用的 experts 替换为 1x1 的占位参数，
    #     以节省显存并保持索引稳定。
    #     假设 forward 中不会访问这些 expert。
    #     """
    #     from collections import Counter

    #     used = set(self.expert_routing_map)
    #     total = set(range(len(self.weight_up)))
    #     unused = sorted(total - used)

    #     if not unused:
    #         print("没有未使用的 expert。")
    #         return

    #     print(f"检测到未使用的 experts：{unused}，将替换为 1x1 占位参数。")

    #     for i in unused:
    #         device = self.weight_gate[i].device
    #         dtype = self.weight_gate[i].dtype

    #         # 占位参数：极小尺寸
    #         self.weight_gate[i] = nn.Parameter(torch.zeros(1, 1, device=device, dtype=dtype))
    #         self.weight_up[i] = nn.Parameter(torch.zeros(1, 1, device=device, dtype=dtype))
    #         self.weight_down[i] = nn.Parameter(torch.zeros(1, 1, device=device, dtype=dtype))

    #         if self.bias_gate is not None:
    #             self.bias_gate[i] = nn.Parameter(torch.zeros(1, device=device, dtype=dtype))
    #             self.bias_up[i] = nn.Parameter(torch.zeros(1, device=device, dtype=dtype))
    #             self.bias_down[i] = nn.Parameter(torch.zeros(1, device=device, dtype=dtype))

    #         # 更新 expert 尺寸（以防其他模块依赖）
    #         self.size_experts[i] = 1

    #     print("所有未使用 expert 已替换为 1x1 占位参数，routing_map 保持不变。")
