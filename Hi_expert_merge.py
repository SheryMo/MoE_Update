...
def assign_experts_to_groups_by_similarity(
    ...
    group = [[random.randint(0, 7) for _ in range(16)] for _ in range(num_layers)]  # 初始化组标签
    # limit = 8
    # # 遍历每一层
...
def map_to_range(data, new_min=0.1, new_max=0.9):
    ...
    return result
# def _merge_all_experts_in_last_three_layers(
#         model,
#         frequency_list,  # Frequency list of experts in the last three layers, shape = (3, num_experts)
#         num_layers=3,    # Always consider the last three layers
#         num_experts=16   # 16 experts per layer
# ):
#     """
#     Merge all experts across the last three layers into a single expert using frequency weights.
#     The parameters of the first expert in the first layer are updated to the merged parameters,
#     and all experts in the last three layers point to this expert.
    
#     Args:
#         model (PhiMoEForCausalLM): The model containing all layers.
#         frequency_list (List[List[float]]): A list of 3 lists, each containing 16 frequency values for the experts.
#         num_layers (int): The number of layers to consider (default is 3, for the last three layers).
#         num_experts (int): The number of experts per layer (default is 16).
    
#     Returns:
#         PhiMoEForCausalLM: The updated model with merged experts in the last three layers.
#     """
#     print(len(frequency_list))
#     assert len(frequency_list) == num_layers, "The frequency list should have a length equal to the number of layers."
#     assert len(frequency_list[0]) == num_experts, "Each layer should have the same number of experts."
    
#     # Convert frequency list to tensor
#     frequency_tensor = torch.tensor(frequency_list, dtype=torch.float32)  # Shape: (num_layers, num_experts)

#     # Normalize the frequency list across the three layers
#     normalized_frequencies = frequency_tensor / (torch.sum(frequency_tensor, dim=1, keepdim=True) )  # Shape: (num_layers, num_experts)
    
#     # Accumulate weighted parameters across all three layers
#     merged_w1, merged_w2, merged_w3 = 0, 0, 0
#     total_weight = 0
    
#     for layer_idx in range(num_layers):
#         for expert_idx in range(num_experts):
#             weight = normalized_frequencies[layer_idx, expert_idx]
#             # Get the corresponding expert across the layers
#             expert = model.model.layers[layer_idx].block_sparse_moe.experts[expert_idx]
            
#             # Accumulate weighted parameters for each expert
#             merged_w1 += expert.w1.weight * weight
#             merged_w2 += expert.w2.weight * weight
#             merged_w3 += expert.w3.weight * weight
#             total_weight += weight

#     # Normalize accumulated weights by the total weight
#     merged_w1 /= (total_weight )
#     merged_w2 /= (total_weight )
#     merged_w3 /= (total_weight )
    
#     # Set the merged parameters to the first expert of the first layer
#     first_expert_idx = 0  # The first expert in the first layer
#     first_layer_expert = model.model.layers[0].block_sparse_moe.experts[first_expert_idx]
    
#     # Use detach() to prevent gradient tracking and avoid in-place operation error
#     first_layer_expert.w1.weight.data = merged_w1.detach()
#     first_layer_expert.w2.weight.data = merged_w2.detach()
#     first_layer_expert.w3.weight.data = merged_w3.detach()

#     # Bind all experts in the last three layers to the first expert
#     for layer_idx in range(num_layers):
#         for expert_idx in range(num_experts):
#             model.model.layers[layer_idx].block_sparse_moe.experts[expert_idx] = model.model.layers[0].block_sparse_moe.experts[first_expert_idx]

#     return model
    
# def merge_experts_in_last_three_layers(model, frequency_list):
#     """
#     Merge all experts in the last three layers into a single expert based on their frequency weights.
...
#         PhiMoEForCausalLM: The updated model with merged experts.
#     """
#     # Merge experts across the last three layers (layers 30, 31, and 32)
#     model = _merge_all_experts_in_last_three_layers(
#         model=model,  # Pass the model instance here
#         frequency_list=frequency_list[29:],  # Frequency list for all three layers
#         num_layers=3,  # Number of layers to merge experts across
#         num_experts=16  # 16 experts per layer
#     )
...
model._model = merge_by_groups_with_usage_frequency_weighting(model._model, frequency_list)
