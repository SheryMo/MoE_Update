...
def cross_layer_expert_merge(model, frequency_list, group, group_idx, layer_idx):
    """
    ...
    for cross_layer_idx in range(num_layer):
        ...
        for expert_idx in range(len(group_layer)):
            if group_layer[expert_idx] != group_idx:
                continue  # Skip experts not in the target group
    ...
    with torch.no_grad():
        for expert_idx in range(len(group[layer_idx])):
            if group[layer_idx][expert_idx] == group_idx:
                expp = expert_idx
                break
        model.model.layers[layer_idx].mlp.experts[expp].up_proj.weight.copy_(accumulated_up_proj_weight)
        model.model.layers[layer_idx].mlp.experts[expp].down_proj.weight.copy_(accumulated_down_proj_weight)
        model.model.layers[layer_idx].mlp.experts[expp].gate_proj.weight.copy_(accumulated_gate_proj_weight)
    ...
    for cross_layer_idx in range(num_layer):
        for expert_idx in range(len(group[cross_layer_idx])):
            if group[cross_layer_idx][expert_idx] != group_idx:
                continue  # Skip experts not in the target group
            model.model.layers[cross_layer_idx].mlp.experts[expert_idx] = model.model.layers[layer_idx].mlp.experts[expp]
...
def adjust_groups_based_on_group_similarity(frequency_list, group):
    """
    Adjust the groupings across layers based on frequency variance similarity between layers.
    ...
    global similarity_group
    num_layers = len(frequency_list)
    num_groups = len(similarity_group)
    notdone = True
    base_sim = 0.8
    while notdone: 
        # Step 2: Compare layers for variance similarity starting from the last layer
        for layer_idx in range(num_layers - 1, 2, -1):
            ...
            total_groups = 0
            for layer_index in range(num_layers):
                current_layer_groups = set(group[layer_index])
                total_groups += len(current_layer_groups)
            if total_groups <= (num_expert * num_layer *args.percent):
                print(f"Total groups {total_groups} exceeded the threshold, stopping comparison.")
                notdone = False
                break
            group_thisLayer = set(group[layer_idx])
            for group_idx in group_thisLayer:
                group_compare = set(set(group[layer_idx - 1]) - set(group_thisLayer))
                group_idx = int(group_idx)
                for compare_group in group_compare:
                    current_variance = similarity_group[group_idx]
                    previous_variance = similarity_group[int(compare_group)]
                    # Step 3: Compute the similarity between the current layer and the previous layer
                    similarity = compute_similarity_between_layers(current_variance, previous_variance)
                    # print(similarity)
                    # Step 4: If similarity exceeds the threshold (0.8), set both groups to -2
                    if similarity >= base_sim:
                        for layerr in range(num_layers):
                            for expert_idx in range(len(group[layerr])):
                                if group[layerr][expert_idx] == group_idx:
                                    group[layerr][expert_idx] = int(compare_group)
        base_sim = base_sim/2
    return group
...
def assign_experts_to_groups_by_similarity(
    ...
    for layer_idx in range(num_layers):
        ...
        while current_group_idx < limit:
            ...
            group[layer_idx][anchor_expert_idx] = int(current_group_idx + accumulated_groupNum)
            anchor_expert_output = layer_expert_output[anchor_expert_idx]
            similarity_group.append(anchor_expert_output)
        ...
        while True:
            ...
            if current_group_idx >= limit:
                break
            # 选择当前未分组的最大频率专家作为定点
            anchor_expert_idx = [idx for idx, value in enumerate(group[layer_idx]) if value == current_group_idx + accumulated_groupNum]
            ...
            while True:
                ...
                if layer_expert_output[best_match_idx][0] == 0:
                    ...
                    continue
                group[layer_idx][best_match_idx] = int(current_group_idx + accumulated_groupNum)
        ...
        if ungrouped_experts:
            for expert_idx in ungrouped_experts:
                group[layer_idx][expert_idx] = int(limit - 1 + accumulated_groupNum)  # 将剩余专家归为最后一组
            ungrouped_experts.clear()  # 清空未分组专家列表
        accumulated_groupNum += limit
    return group
...
def merge_by_groups_with_usage_frequency_weighting(
    ...
    print(len(similarity_group))
    print(max(map(max,group)))
    
    group = adjust_groups_based_on_group_similarity(frequency_list, group)
    
    # print(group)
    # print("1")
    num_layers = len(frequency_list)
    layer_idx = 0
    for group_idx in set([layer for sublist in group for layer in sublist]):
        trans = True
        for layer_index in range(num_layers):
            if group_idx in group[layer_index] and trans:
                print(f"Layer {layer_index} - Group {group_idx}")
                trans = False
                model = cross_layer_expert_merge(model, frequency_list, group,group_idx, layer_idx)
